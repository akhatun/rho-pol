...
void CorrectedMass(){
  ...
  TFile* file   = new TFile("run3-MC/AxE/AxE-run2.root", "READ");
  TFile* file2  = new TFile("run3-MC/Polarisation-results.root", "READ");
  TH1F* AxECosTheta[16];
  TH1F* AxEPhi[12];
  ...
  {
    MassCosTheta[i] = (TH1F*) file2->Get(Form("polarisation-rho/hMassCosTheta_%i", i));
  ...
  {
    MassPhi[i] = (TH1F*) file2->Get(Form("polarisation-rho/hMassPhi_%i", i));
  ...
  TFile *SavingFile = new TFile("run3-MC/AxE/Corrected-with-run2.root", "RECREATE");
  for (Int_t i = 0; i < 16; i++)
  {
...
void BeautifyHistoCosTheta(TH1* histogram, int i){
  ...
  gPad->SaveAs(Form("run3-MC/AxE/Corr-costheta-%i.pdf", i), "recreate");
...
void BeautifyHistoPhi(TH1* histogram, int i){
  ...
  gPad->SaveAs(Form("run3-MC/AxE/Corr-phi-%i.pdf", i), "recreate");
...
void BeautifyHistoCosThetaRaw(TH1* histogram, int i){
  ...
  if(i == 14) latex10->DrawLatex(0.2,0.72, " 0.55 < cos#theta <  0.7");
  if(i == 15) latex10->DrawLatex(0.2,0.72, " 0.7  < cos#theta <  1");
  gPad->SaveAs(Form("run3-MC/RawData/Raw-costheta-%i.pdf", i), "recreate");
...
void BeautifyHistoPhiRaw(TH1* histogram, int i){
  ...
  if(i == 10) latex10->DrawLatex(0.2,0.72, "10#times#frac{2#pi}{12} < #varphi < 11#times#frac{2#pi}{12}");
  if(i == 11) latex10->DrawLatex(0.2,0.72, "11#times#frac{2#pi}{12} < #varphi < 12#times#frac{2#pi}{12}");
  gPad->SaveAs(Form("run3-MC/RawData/Raw-phi-%i.pdf", i), "recreate");
